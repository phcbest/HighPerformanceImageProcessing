package org.phcbest.imagecrop.view;
...
import org.phcbest.imagecrop.R;
import org.phcbest.imagecrop.callback.OverlayViewChangeListener;
import org.phcbest.imagecrop.util.RectUtils;
...
public class OverlayView extends View {
    ...
    private int mCropRectMinHeight = 100;
    private int mCropRectMinWidth = 100;
    private int mCropRectCornerTouchAreaLineLength;

    private OverlayViewChangeListener mCallback;

    private boolean mShouldSetupCropBounds;
    public boolean isFreeStyleLockAspectRatio;
    ...
    public void setCropRectMinWH(Pair<Integer, Integer> wh) {
        this.mCropRectMinWidth = wh.first;
        this.mCropRectMinHeight = wh.second;
    }
    ...
    private void updateCropViewRect(float touchX, float touchY) {
        ...
        if (isFreeStyleLockAspectRatio && (mTempRect.width() / mTempRect.height()) != mTargetAspectRatio) {
            Log.i(TAG, "updateCropViewRect: 宽高比" + mTempRect.width() / mTempRect.height());
            Log.i(TAG, "updateCropViewRect: 要求宽高比" + mTargetAspectRatio);
            Log.i(TAG, "updateCropViewRect: ================================================");
            //基于宽度矫正高度
            float newHeight = mTempRect.width() / mTargetAspectRatio;
            if (mCurrentTouchCornerIndex == 0 || mCurrentTouchCornerIndex == 1) {
                mTempRect.top = mTempRect.bottom - newHeight;
            } else if (mCurrentTouchCornerIndex == 2 || mCurrentTouchCornerIndex == 3) {
                mTempRect.bottom = mTempRect.top + newHeight;
            }
